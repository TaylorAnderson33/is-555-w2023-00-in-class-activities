...
df %>% 
  group_by(is.na(age)) %>%
  summarize(across(everything(), ~mean(.x, na.rm = T))) # the mean is our survival rate in this case

# fill in missing age values, check our work
df <- df %>% 
  mutate(age = if_else(is.na(age), mean(age, na.rm = T), age)) 
  # count(is.na(age)) <- Check to make to make sure count is right for NA values

# now handle embarked, this time using replace_na()
# Again, check our work
df %>%
  mutate(embarked = replace_na(embarked, 'Unknown'))
  #count(embarked) <- Check count to make sure it's correct
